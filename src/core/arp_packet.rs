use std::{error::Error, fmt::{self, Display}, net::Ipv4Addr};
...
impl fmt::Display for ArpPacketError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ArpPacketError::InvalidPacketSize => write!(f, "Invalid ARP packet size"),
            ArpPacketError::InvalidOperation => write!(f, "Invalid ARP operation"),
            ArpPacketError::InvalidHardwareType => write!(f, "Invalid hardware type"),
            ArpPacketError::InvalidProtocolType => write!(f, "Invalid protocol type"),
        }
    }
}

impl Error for ArpPacketError {}
...
impl ArpHardwareType {
    pub fn from_u16(value: u16) -> Result<Self, ArpPacketError> {
        match value {
            1 => Ok(ArpHardwareType::Ethernet),
            _ => Err(ArpPacketError::InvalidHardwareType),
        }
    }
}
...
impl ArpProtocolType {
    pub fn from_u16(value: u16) -> Result<Self, ArpPacketError> {
        match value {
            0x0800 => Ok(ArpProtocolType::Ipv4),
            _ => Err(ArpPacketError::InvalidProtocolType),
        }
    }
}
...
impl ArpOperation {
    pub fn from_u16(value: u16) -> Result<Self, ArpPacketError> {
        match value {
            ...
            _ => Err(ArpPacketError::InvalidOperation),
        }
    }
}
...
impl ArpPacket {
    ...
    pub fn to_bytes(&self) -> [u8; ARP_PACKET_SIZE] {
        ...
        bytes[index..index+6].copy_from_slice(&self.sender_mac.octets());
        index += 6;

        // Sender protocol address (4 bytes)
        bytes[index..index+4].copy_from_slice(&self.sender_ip.octets());
        index += 4;

        // Target hardware address (6 bytes)
        bytes[index..index+6].copy_from_slice(&self.target_mac.octets());
        index += 6;

        // Target protocol address (4 bytes)
        bytes[index..index+4].copy_from_slice(&self.target_ip.octets());

        bytes
    }

    pub fn from_bytes(bytes: &[u8; ARP_PACKET_SIZE]) -> Result<Self, ArpPacketError> {
        let mut index = 0;

        // Parse hardware type (2 bytes)
        let hw_type = u16::from_be_bytes([bytes[index], bytes[index+1]]);
        let hardware_type = ArpHardwareType::from_u16(hw_type)?;
        index += 2;

        // Parse protocol type (2 bytes)
        let proto_type = u16::from_be_bytes([bytes[index], bytes[index+1]]);
        let protocol_type = ArpProtocolType::from_u16(proto_type)?;
        index += 2;
    ...
    pub fn create_request(
        ...
        let target_mac = MacAddr::new(0x00, 0x00, 0x00, 0x00, 0x00, 0x00);
...
mod tests {
    ...
    fn test_hardware_type_from_u16() {
        assert_eq!(ArpHardwareType::from_u16(1), Ok(ArpHardwareType::Ethernet));
        assert_eq!(ArpHardwareType::from_u16(2), Err(ArpPacketError::InvalidHardwareType));
    }

    #[test]
    fn test_protocol_type_from_u16() {
        assert_eq!(ArpProtocolType::from_u16(0x0800), Ok(ArpProtocolType::Ipv4));
        assert_eq!(ArpProtocolType::from_u16(0x0806), Err(ArpPacketError::InvalidProtocolType));
    }
    ...
    fn test_operation_from_u16() {
        ...
        assert_eq!(ArpOperation::from_u16(3), Err(ArpPacketError::InvalidOperation));
    }
    ...
    fn test_arp_packet_creation() {
        ...
        assert_eq!(packet.hardware_type(), ArpHardwareType::Ethernet);
        assert_eq!(packet.protocol_type(), ArpProtocolType::Ipv4);
    ...
    fn test_arp_request_creation() {
        ...
        assert_eq!(packet.target_mac(), MacAddr::new(0x00, 0x00, 0x00, 0x00, 0x00, 0x00));
    }
    ...
    fn test_arp_packet_from_bytes_invalid_hardware_type() {
        ...
        assert_eq!(ArpPacket::from_bytes(&bytes), Err(ArpPacketError::InvalidHardwareType));
    }
    ...
    fn test_is_request_for() {
        ...
        let other_ip = Ipv4Addr::new(192, 168, 1, 200);
    ...
    fn test_error_display() {
        assert_eq!(format!("{}", ArpPacketError::InvalidPacketSize), "Invalid ARP packet size");
        assert_eq!(format!("{}", ArpPacketError::InvalidOperation), "Invalid ARP operation");
        assert_eq!(format!("{}", ArpPacketError::InvalidHardwareType), "Invalid hardware type");
        assert_eq!(format!("{}", ArpPacketError::InvalidProtocolType), "Invalid protocol type");
    }
}