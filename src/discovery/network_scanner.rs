...
use crate::core::{Cidr, MAC_ADDRESS_BROADCAST};
...
impl NetworkScanner {
    pub fn new(interface_name: String, timeout: Duration) -> Self {
        NetworkScanner { interface_name, timeout }
    }
    ...
    pub async fn scan_ip(&self, target_ip: Ipv4Addr) -> Result<Option<DiscoveredDevice>, ScanError> {
        ...
        if let Some((mac_address, ipv4_address)) = self.send_arp_request(target_ip).await? {
            Ok(Some(DiscoveredDevice { ip: ipv4_address, mac: mac_address, response_time: start_time.elapsed() }))
        } else {
            Ok(None)
        }
    }
    ...
    pub async fn scan_network(&self, network: &Cidr) -> Result<Vec<DiscoveredDevice>, ScanError> {
        ...
        for ip in network.network_addresses() {
            if let Some(device) = self.scan_ip(ip).await? {
                devices.push(device);
            }
        }
...
mod tests {
    ...
    async fn test_scan_ip() {
        let scanner = NetworkScanner::new("eth0".to_string(), Duration::from_secs(5));
        let target_ip = Ipv4Addr::new(192, 168, 1, 1);
        match scanner.scan_ip(target_ip).await {
            Ok(Some(device)) => {
                println!("Discovered device: IP: {}, MAC: {}, Response Time: {:?}", device.ip, device.mac, device.response_time);
            },
            Ok(None) => println!("No device found at {}", target_ip),
            Err(e) => eprintln!("Error scanning IP: {:?}", e),
        }
    }
    ...
    async fn test_scan_network() {
        ...
        match scanner.scan_network(&cidr).await {
            ...
            Err(e) => eprintln!("Error scanning network: {:?}", e),
        }
    }

    #[test]
    fn test_build_arp_request_packet() {
        let scanner = NetworkScanner::new("eth0".to_string(), Duration::from_secs(5));
        let local_ip = Ipv4Addr::new(192, 168, 1, 100);
        let local_mac = MacAddr::new(0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff);
        let target_ip = Ipv4Addr::new(192, 168, 1, 1);
        let packet = scanner.build_arp_request_packet(local_ip, local_mac, target_ip).unwrap();
        assert_eq!(packet.len(), 42); // 14 bytes Ethernet + 28 bytes ARP

        let ethernet_packet = EthernetPacket::new(&packet).unwrap();
        assert_eq!(ethernet_packet.get_destination(), MAC_ADDRESS_BROADCAST);
        assert_eq!(ethernet_packet.get_source(), local_mac);
        ...
        assert_eq!(arp_packet.get_sender_proto_addr(), local_ip);
        assert_eq!(arp_packet.get_target_proto_addr(), target_ip);
        assert_eq!(arp_packet.get_sender_hw_addr(), local_mac);
    }
}